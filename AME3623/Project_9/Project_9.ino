...
{
    //initializing state
  static State state = STATE_START_P9;
  //integer counter for project 9
  static uint16_t time_counter_p9;
  //temporary variable for magnitudes
  static float ramping_up = 0.0;
  //increment rate
  const float RAMP_INCREMENT_P5 = 1.78;
  //temporary variable for STATE_TURN_LEFT_P9
  ...
  switch (state) {
    ...
    case STATE_START_P9:
      ...
      if (GPIOD_PDIR & 0x2)
      {
        //set the ramping_up
        ramping_up = 0.0;
        //set the new state to wait for 5s
        state = STATE_WAIT_P9;
    ...
    //wait state
    case STATE_WAIT_P9:
      //check the counter to see if 5 seconds has past
      if (time_counter_p9 >= 100) {
        //when done waiting, set state to STATE_MIDDLE_FAN_RAMP_UP_P9 to ramp up
        state = STATE_MIDDLE_FAN_RAMP_UP_P9;
      }
    ...
    case STATE_MOVE_FORWARD_P9:

      // Set the velocity goal to 1 for the x and 0 for the y.
      velocity_goal[0] = 0.10;
      velocity_goal[1] = 0;
      //if distance to the wall is less than 15 cm, run this code 
      if (distance_sensors[1] < 15) {
        //set state to STATE_STOPPING
        state = STATE_STOPPING;
      }
      break;

    //state for stopping temporarily
    case STATE_STOPPING:
      // Set the velocity goal to 0.
      velocity_goal[0] = 0;
      velocity_goal[1] = 0;
      //if the motion for x,y is less than 0.01,run this code
      if (motion[0] < 0.01 && motion[1] < 0.01) {
        //initialize firstTime for the next state
        firstTime=0;
        //set state to STATE_TURN_LEFT_P9, after stopping temporarily
        state = STATE_TURN_LEFT_P9;
      }
      break;

    //state for turning left
    case STATE_TURN_LEFT_P9:
      ...
      velocity_goal[0] = 0;
      velocity_goal[1] = 0;
      //if this is the first time for this state, run this code
      if(firstTime==0)
      {     
          //store current orientation to make left turn correctly
          oldOrientation=IMU.yaw;
          //set firstTime 1 not to run this code again
          firstTime=1;
       }
       ...
       {    
          ...
          if (abs(ret) < 5 ) {
           ...
           state = STATE_MOVE_FORWARD_2_P9;
          }
        }
      break;

    //state for moving forward after turning left
    case STATE_MOVE_FORWARD_2_P9:
      // Set the velocity goal to 1 for the x and 0 for the y.
      velocity_goal[0] = 0.10;
      velocity_goal[1] = 0;
            //if distance to the wall is less than 15 cm, run this code 
      if (distance_sensors[1] < 15) {
        //set state STATE_STOPPED, after moving forward
        state = STATE_STOPPED;
      }
      break;

    //state for stopping including shutting down all fans.
    case STATE_STOPPED:
      //set velocity goal to 0
      velocity_goal[0] = 0;
      velocity_goal[1] = 0;
      //set magnitude 0 for cenral fan, to shut down
        set_lift_fan_magnitude(0);
      //set heading_goal to current orientation to stop lateral fans
      heading_goal= IMU.yaw;
  ...
  /*
    ...
    case STATE_RAMP_CENTER_UP_P5:
      ...
      if (ramp_state > 177)
      {
        ...
        state = STATE_HOVER_P15;
        Serial.println("I'm hovering 15s");
      }
      else
      {
        // Adjust the magnitude
    ...
    case STATE_HOVER_P15:
      // Check the counter to see if 15 seconds has past
      if (counter < 300)
      {
        counter++;
        //Keep the state in STATE_HOVER_P15
        state = STATE_HOVER_P15;
      ...
      {
        //When done waiting, set to STATE_VELOCITY_GOAL_X
        state =   STATE_VELOCITY_GOAL_X;
        Serial.println("I'm going forward");
        counter=0;
      }
      break;

    //Set velocity goal to something in the x direction
    case STATE_VELOCITY_GOAL_X:
      // Check the counter to see if 10 seconds has past
      if (counter < 200)
      {
        counter++;
        //Keep the state in   STATE_VELOCITY_GOAL_X
        //state = STATE_VELOCITY_GOAL_X;
        //Set the lift fan to the same power level
        set_lift_fan_magnitude(ramp_state);
        // Set the velocity goal to 1 for the x and 0 for the y.
      ...
      {
        //When done waiting, set to STATE_Hover_P10
        state =   STATE_HOVER_P10;
        Serial.println("I'm hovering 10s");
        counter=0;
      }
      break;

    //WAIT state for 10 seconds
    case   STATE_HOVER_P10:
      // Check the counter to see if 10 seconds has past
      if (counter < 200)
      {
        counter++;
      ...
      else
      {
        ...
        {
          secondLoop=1;
          //When done waiting, set to STATE_VELOCITY_GOAL_Y
          state =   STATE_VELOCITY_GOAL_Y;
    ...
    //Set velocity goal to something in the Y direction
    case STATE_VELOCITY_GOAL_Y:
      // Check the counter to see if 10 seconds has past
      if (counter < 200)
      {
        counter++;
        //Keep the state in   STATE_VELOCITY_GOAL_Y
        //state = STATE_VELOCITY_GOAL_Y;
        //Set the lift fan to the same power level
        set_lift_fan_magnitude(ramp_state);
        // Set the velocity goal to 1 for the x and 0 for the y.
        velocity_goal[0]=0;
        velocity_goal[1]=0.10;
      }
      else
      {
        //When done waiting, set to STATE_VELOCITY_GOAL_X
